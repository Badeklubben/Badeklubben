...
export default function Form({ form }: { form: BryggeskjemaDocument }) {
    ...
    return (
        <div key={form.id}>
            <h2>{form["batch-navn"] ? form["batch-navn"] : "Tittel"}</h2>
            <p>Batch Nummer: {form["batch-nr"]}</p>
            <p>
                Brygge dato: {form.bryggedato ? formatDate(form.bryggedato) : 0}
            </p>
            <p>Tappe dato: {form.tappedato ? formatDate(form.tappedato) : 0}</p>
            <p>ABV: {form.abv}%</p>
