        ...
        <div className="relative flex flex-col w-full rounded-[30px] overflow-hidden shadow-[0px_5px_5px_rgba(0,0,0,0.3)] transition-all duration-200 hover:scale-[1.02] hover:shadow-lg">
            ...
            {isExternal ? (
                <a
                    ...
                    className="flex h-28 w-full items-center justify-center relative"
                    style={{ backgroundColor: color }}
                >
                    {iconArea}
                    <FaExternalLinkAlt className="absolute top-3 right-3 w-3 h-3 text-white opacity-60" />
                </a>
            ) : (
                <Link
                    href={href}
            ...
            <div className="bg-white px-5 py-4 flex flex-col gap-2 flex-1">
                {isExternal ? (
                    <a href={href} target="_blank" rel="noopener noreferrer">
                        <div className="font-bold text-sm hover:underline flex items-center gap-1.5">
                            {name}
                            <FaExternalLinkAlt className="w-2.5 h-2.5 text-gray-400 shrink-0" />
                        </div>
                    </a>
                ) : (
                    <Link href={href}>
                ...
                {/* External links — only shown on internal cards */}
                {!isExternal && (project?.githubUrl || project?.liveUrl) && (
