...
import {
  loadImage,
  embedPasswordInImageData,
  extractPasswordFromImageData,
} from "./stegno_logic";
...
const SteganographyForm: React.FC = () => {
  ...
  const [enteredSecurityPassword, setEnteredSecurityPassword] =
    useState<string>("");
  ...
  const handleSecurityPasswordChange = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    setSecurityPassword(event.target.value);
  };

  const handleEnteredSecurityPasswordChange = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
  ...
  const embedPassword = async () => {
    ...
    if (ctx) {
      ...
      const newData = await embedPasswordInImageData(
        imgData,
        password,
        securityPassword,
      );
  ...
  const extractPassword = async () => {
    ...
    if (ctx) {
      ...
      const password = await extractPasswordFromImageData(
        imgData,
        enteredSecurityPassword,
      );
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.inputWrapper}>
        ...
        {showTooltip && (
          <div className={styles.tooltip}>
            This password will be required to extract the hidden password.
          </div>
        )}
      </div>
      <div>
        <button onClick={embedPassword} className={styles.button}>
          Embed Password
        </button>
      </div>
      <div>
      ...
      <div>
        <button onClick={extractPassword} className={styles.extractButton}>
          Extract Password
        </button>
      </div>
      {extractedPassword && (
        <p className={styles.extractedPassword}>
          Extracted Password: {extractedPassword}
        </p>
      )}
    </div>
  );
